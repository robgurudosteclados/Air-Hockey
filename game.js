(function(){
  ...
  // FÍSICA
  // =====================================================================
  const FIELD_W = 480, FIELD_H = 760, GOAL_W = 174, PUCK_R = 15, PADDLE_R = 32; // GOAL_W reduzido ~2mm de cada lado (~8 unidades por lado)
  const GOAL_X0 = (FIELD_W-GOAL_W)/2, GOAL_X1 = (FIELD_W+GOAL_W)/2;
  const MAX_SPEED = 1650;
  ...
  function resolveCollision(puck, paddle){
    ...
    if(dist < minDist){
      ...
      puck.vx += paddle.vx*0.95; puck.vy += paddle.vy*0.95;
  ...
  function updateAIPaddle(paddle, puck, side, difficulty, dt){
    ...
    // no difícil, a IA só avança quando o disco realmente cruza pra defesa dela (fica
    // mais recuada/defensiva enquanto o disco está do outro lado); nos demais níveis,
    // mantém a zona de alcance mais generosa de antes.
    const reachFraction = difficulty==='hard' ? 0.5 : 0.62;
    const inReachZone = side==='top' ? puck.y < FIELD_H*reachFraction : puck.y > FIELD_H*(1-reachFraction);

    let targetX, targetY;
    if(puckHeadingToAI || inReachZone){
      const predX = puck.x + puck.vx*prof.predictLead;
      const predY = puck.y + puck.vy*prof.predictLead*0.5;
      // a posição-alvo sempre persegue o disco de verdade, em qualquer ponto do campo
      // (inclusive nos extremos laterais) — a mira nunca deve impedir o paddle de alcançar
      // o disco. Só perto o bastante pra realmente tocar nele é que aplicamos um pequeno
      // desvio (não uma mistura) que empurra o contato pro lado certo, criando o ângulo.
      targetX = predX;
      const distToPuck = Math.hypot(puck.x-paddle.x, puck.y-paddle.y);
      const CONTACT_RANGE = 60;
      if(distToPuck < CONTACT_RANGE){
        const closeness = 1 - distToPuck/CONTACT_RANGE; // 0 longe, 1 quase encostando
        const maxOffset = 26 * (0.4 + prof.aggression*0.6);
        const aimDir = paddle.aimTargetX < predX ? -1 : 1; // aborda pelo lado oposto ao alvo de mira
        targetX += aimDir * maxOffset * closeness;
      }
  ...
  // regra do Gol contra (2+ toques do defensor na própria defesa invalidam o gol) é sempre ativa
  // (som ambiente durante a partida agora é obrigatório — sem opção de ligar/desligar)
  ...
  function beginMatch(cfg, starter, humanSide){
    ...
    match = {
      ...
      puckStallTimer:0, aiZoneStallTimer:0, wallLoopSide:null, wallLoopCount:0
    };
  ...
  function gameLoop(ts){
    ...
    if(!match.frozenForGoal){
      ...
      if(match.pointerActive && match.pointerTargetX!==null){
        ...
        const rawX = Math.max(PADDLE_R, Math.min(FIELD_W-PADDLE_R, match.pointerTargetX));
        const rawY = Math.max(yMin, Math.min(yMax, match.pointerTargetY));
        // limita o quanto o paddle realmente pode se deslocar num único quadro — sem isso,
        // um clique/toque em qualquer ponto distante do campo teleportava o paddle na hora
        // pra lá (dava pra "respawnar" na frente do disco sem precisar se deslocar de verdade).
        const HUMAN_MAX_SPEED = 2400; // ainda bem rápido/responsivo, só não é teleporte instantâneo
        const ddx = rawX-humanPaddle.x, ddy = rawY-humanPaddle.y;
        const ddist = Math.hypot(ddx,ddy) || 0.0001;
        const dstep = Math.min(ddist, HUMAN_MAX_SPEED*dt);
        const newX = humanPaddle.x + (ddx/ddist)*dstep;
        const newY = humanPaddle.y + (ddy/ddist)*dstep;
        if(dt>0){
  ...
  // Detecta muitas rebatidas seguidas na mesma parede lateral, dentro do campo de defesa
  // da IA, sem o disco cruzar pro ataque — vale tanto pra rebatidas retilíneas (sempre no
  // mesmo ponto) quanto pra rebatidas em pontos diferentes da mesma parede (o paddle
  // "compensando" e mantendo o disco preso ali). 10 rebatidas seguidas já dispara o realinhamento.
  const WALL_LOOP_LIMIT = 10;
  function checkWallLoop(){
    const p = match.objs.puck;
    const inAIHalf = match.aiSide==='top' ? p.y < FIELD_H/2 : p.y > FIELD_H/2;
    if(!inAIHalf){
      match.wallLoopSide = null; match.wallLoopCount = 0;
      return;
    }
    ...
    if(match.wallLoopCount >= WALL_LOOP_LIMIT){
      repositionAfterConcede(match.aiSide);
      document.getElementById('matchStatus').textContent = 'Anti-travamento automático: muitas rebatidas seguidas na mesma parede da defesa — realinhado.';
      match.wallLoopSide = null; match.wallLoopCount = 0;
    }
  }
  ...
  function handleGoal(scoredSide){
    ...
    const isInvalidOwnGoal = concedingPaddle.touchesInOwnHalf >= 2;
  ...
  const VICTORY_FANFARE_DURATION = 10.5; // duração aproximada da fanfarra de campeão, em segundos
  const MENU_MUSIC_RESUME_GAP = 2; // intervalo silencioso antes da música do menu voltar
      ...
      if(result.done){
        setTimeout(()=>{
          if(result.titleWon){
            ...
            setTimeout(()=> MenuMusic.start(), (VICTORY_FANFARE_DURATION+MENU_MUSIC_RESUME_GAP)*1000);
          } else {
