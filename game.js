(function(){
  ...
  function launchMatch(cfg){
    ...
    document.getElementById('antiStallBtn').classList.remove('hidden');
  ...
  function beginMatch(cfg, starter, humanSide){
    ...
    match = {
      ...
      puckStallTimer:0, aiZoneStallTimer:0
    };
  ...
  let antiStallCooldownUntil = 0;
  function manualUnstick(){
    if(!match || !match.running || match.frozenForGoal) return;
    const now = performance.now();
    if(now < antiStallCooldownUntil) return;
    const nearTop = match.objs.puck.y < FIELD_H/2;
    repositionAfterConcede(nearTop ? 'top' : 'bottom');
    document.getElementById('matchStatus').textContent = 'Anti-travamento acionado: disco e paddles realinhados.';
    antiStallCooldownUntil = now + 4000;
    const btn = document.getElementById('antiStallBtn');
    btn.disabled = true;
    setTimeout(()=>{ btn.disabled = false; }, 4000);
  }
  document.getElementById('antiStallBtn').addEventListener('click', manualUnstick);
  ...
  // sofrer um gol — usado tanto no gol de verdade quanto no botão manual anti-travamento.
  ...
  function showMatchEndBanner(outcome){
    ...
    sub.textContent = `${match.cfg.humanTeam} ${match.scoreHuman} x ${match.scoreAI} ${match.cfg.aiTeam}`;
    overlay.classList.remove('hidden');
  ...
  function finishMatchFlow(outcome){
    ...
    document.getElementById('antiStallBtn').classList.add('hidden');
    ...
    if(cfg.contextType==='tournament'){
      ...
      if(result.done){
        setTimeout(()=>{
          if(result.titleWon){
            showChampionBanner('Campeão do Torneio!', 'Você é o campeão do Torneio, jogando pelo '+cfg.humanTeam+'!', true, cfg.humanTeam);
      ...
      if(result.seasonOver){
        setTimeout(()=>{
          if(result.champion===cfg.humanTeam){
            showChampionBanner('Campeão da Liga!', 'Você é o campeão da Liga, jogando pelo '+cfg.humanTeam+'!', true, cfg.humanTeam);
            MusicEngine.playVictoryFanfare();
          } else {
