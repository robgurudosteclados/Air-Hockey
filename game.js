(function(){
  ...
  function beginMatch(cfg, starter, humanSide){
    ...
    match = {
      ...
      puckStallTimer:0, aiZoneStallTimer:0, wallLoopSide:null, wallLoopCount:0, wallLoopY:null, wallSideLoopSide:null, wallSideLoopCount:0
    };
  ...
  // Rede de segurança mais ampla: mesmo quando o disco bate em pontos DIFERENTES da
  // parede (o paddle "compensando" e mantendo o disco preso na própria defesa), se forem
  // 15 rebatidas seguidas na MESMA parede sem o disco cruzar pro ataque, também realinha.
  const WALL_SIDE_LOOP_LIMIT = 15;
  function checkWallSideLoop(){
    const p = match.objs.puck;
    const inAIHalf = match.aiSide==='top' ? p.y < FIELD_H/2 : p.y > FIELD_H/2;
    if(!inAIHalf){
      match.wallSideLoopSide = null; match.wallSideLoopCount = 0;
      return;
    }
    let wallHit = null;
    if(Math.abs(p.x - PUCK_R) < 0.6 && p.vx > 0) wallHit = 'left';
    ...
    if(!wallHit) return;
    if(wallHit === match.wallSideLoopSide){
      match.wallSideLoopCount++;
    } else {
      match.wallSideLoopSide = wallHit;
      match.wallSideLoopCount = 1;
    }
    if(match.wallSideLoopCount >= WALL_SIDE_LOOP_LIMIT){
      repositionAfterConcede(match.aiSide);
  ...
  function repositionAfterConcede(concedingSide){
    ...
    match.wallSideLoopSide = null;
    match.wallSideLoopCount = 0;
    match.objs.top.touchesInOwnHalf = 0;
    match.objs.bottom.touchesInOwnHalf = 0;
    match.objs.top.wasTouching = false;
    match.objs.bottom.wasTouching = false;
