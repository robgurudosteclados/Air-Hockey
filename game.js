(function(){
  ...
  function resolveCollision(puck, paddle){
    ...
    if(dist < minDist){
      ...
      puck.vx += paddle.vx*1.0; puck.vy += paddle.vy*1.0;
  ...
  function gameLoop(ts){
    ...
    if(!match.frozenForGoal){
      ...
      if(match.pointerActive && match.pointerTargetX!==null){
        ...
        // o paddle só chega a se mover porque o toque começou grudado nele (veja
        // GRAB_RADIUS em onPointerDown) — a partir daí, segue o dedo 1:1 de verdade,
        // sem limite artificial de velocidade (é o próprio deslize real que dá a força do chute).
        if(dt>0){
  ...
  const GRAB_RADIUS = 95; // toque precisa começar perto do paddle pra "grudar" nele
  ...
  function onPointerDown(e){
    ...
    const [cx,cy] = clientToCanonical(p.clientX, p.clientY);
    const paddle = match.humanSide==='top'?match.objs.top:match.objs.bottom;
    const dist = Math.hypot(cx-paddle.x, cy-paddle.y);
    if(dist > GRAB_RADIUS) return; // toque longe do paddle: não "chama" ele até lá, ignora
